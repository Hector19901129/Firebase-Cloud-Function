...
var leave, student, parents, html, leave_id;
...
function sendApprovalEmail(leaveObject, studentObject, parentObject, htmlText, momOrDad) {
	...
	htmlText = htmlText.replace('studentId1', leave.student_id);
	htmlText = htmlText.replace('studentId1', leave.student_id);
	htmlText = htmlText.replace('studentId2', leave.student_id);
	htmlText = htmlText.replace('studentId2', leave.student_id);
...
// 	var leaveId = '-LXHTDD6NFwHhZRRbqPL';
// 	var studentId = '6GqE3j0EnLhKsf8tpzYU7YaecHo2';

// 	var result = await admin.database().ref(`/newleaves/${studentId}/${leaveId}`).once('value', function (snapshot) {
...
// 		console.log('leave', leave.student_id);
...
// 				}).then((dad) => {
// 					parents.push(dad);
// 					return sendApprovalEmail(leave, student, parents[0], html, "mom");
// 				}).then((obj) => {
// 					return sendApprovalEmail(leave, student, parents[1], html, "dad");
// 				});
// 			}
// 		}
...
function updateTheLeaveInLeavesNode(status, leaveId){
      admin.database().ref('leaves/' + leaveId).once('value', (snapshot) => {
      				console.log('updating leaves node');

			if (snapshot.exists()) {
			admin.database().ref('leaves/' + leaveId + '/status').set(status);
			}
		});
}
...
exports = module.exports = functions.database.ref('/newleaves/{studentId}/{leaveId}').onWrite((snapshot, context) => {
	...
	if (snapshot.before._data !== null && 
		...
		var oldStatus = snapshot.before._data.status&0x0FFF;
		var newStatus = snapshot.after._data.status&0x0FFF;
   ...
   if (snapshot.before._data !== null) {
   // update staus in any case to be have the same values
   		updateTheLeaveInLeavesNode(leave.status, leave_id);

   /*
   // added in February 28, 2019
        var oldStatus = snapshot.before._data.status&0x0FFF;
        var oldMomApprovalStatus = (oldStatus&0x0F0)>>4;
	    var oldDadApprovalStatus = (oldStatus&0x00F);

        //check if status value changed due to request fro approval Email
		if(oldMomApprovalStatus !== momApprovalStatus) {
			// update the other nodes for leaves 
				console.log('called the cloud function when make action on approved mail');
					updateTheLeaveInLeavesNode(leave.status, leave_id);


		}else if (oldDadApprovalStatus !== dadApprovalStatus){
				console.log('called the cloud function when make action on approved mail');
				updateTheLeaveInLeavesNode(leave.status, leave_id);
	...
	// Sending push notification
	{
		if (facultyApprovalStatus === 1) {
			...
			return getStudentPromise(leave.student_id).then((studentObject) => {
				student = studentObject;
				return sendPushTo(student.fcmToken, "Leave Request", 
					"Your " + leave.type + " has been declined by faculty.");
			});	
		}else if (momApprovalStatus === 1 || 
			dadApprovalStatus === 1) {
			...
			return getStudentPromise(leave.student_id).then((studentObject) => {
				...
				return sendPushTo(student.fcmToken, "Leave Request Declined", 
					"Your " + leave.type + " has been declined by " + 
					(parent.first_name) + " " + (parent.last_name) + ".");
			});	
		}
		...
		if (facultyApprovalStatus === 2 &&
			...
			return getStudentPromise(leave.student_id).then((studentObject) => {
				...
				student = studentObject;
				return sendPushTo(student.fcmToken, "Leave Request Approved", 
					"Your " + leave.type + " has been approved.");
			});	
		}
	}
